...
/** Modify this function to use atomics. */
__global__ void floyd_warshall_gpu_atomics(size_t n, size_t k) {
}
...
int main(int argc, char** argv) {
  ...
  long agpu_ms = benchmark_one_ms([&]{
    for(int k = 0; k < n; ++k) {
      floyd_warshall_gpu_atomics<<<num_blocks, threads_per_block>>>(n, k);
      CUDIE(cudaDeviceSynchronize());
    }
  });
  std::cout << "GPU (atomics): " << agpu_ms << " ms" << std::endl;
