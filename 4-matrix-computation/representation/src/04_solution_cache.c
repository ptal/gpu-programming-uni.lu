...
static void ddot_multiplication( matrix const* A, vector const* x )
{
  ...
  for ( int i = 0; i < y.nrows; ++i ) {
    double const* const a = &(A->data[i]);
    int const inca = A->ld;
    y.data[i] = cblas_ddot( y.nrows, a, inca, x->data, x->inc );
  }
...
static void daxpy_multiplication( matrix const* A, vector const* x )
{
  ...
  for ( int i = 0; i < y.nrows; ++i ) {
    double const* const a = &(A->data[i * A->ld]);
    int const inca = 1;
    double const alpha = x->data[i];
    cblas_daxpy( y.nrows, alpha, a, inca, y.data, y.inc );
  }
  clock_t diff = clock() - start;
  double const duration = (double) diff / CLOCKS_PER_SEC;
