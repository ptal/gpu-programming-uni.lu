...
static void get_aligned_M( matrix* const A, int const m, int const n )
{
  ...
  A->ld = get_ld( A->nrows, AVX512ALIGN / sizeof(double) );
  A->size = A->ld * A->ncols;

  // Use aligned_alloc to get aligned memory
  A->data = (double*) aligned_alloc( AVX512ALIGN, (A->size) * sizeof(double) );
  if ( A->data == NULL ) {
    fprintf( stderr, "Memory allocation for matrix A failed.\n" );
    exit( EXIT_FAILURE );
  }
...
static void get_aligned_v( vector* const x, int const n )
{
  ...
  // Use aligned_alloc to get aligned memory
  x->data = (double*) aligned_alloc( AVX512ALIGN, (x->size) * sizeof(double) );
  if ( x->data == NULL ) {
    fprintf( stderr, "Memory allocation for matrix A failed.\n" );
    exit( EXIT_FAILURE );
  }
